...
router.get("/", authenticateToken, async (req, res) => {
  try {
    ...
    else if (read_only === "1" || read_only === "true") params.status = "read";
    ...
    if (before_published_at) {
      params.before = Math.floor(
        new Date(before_published_at).getTime() / 1000,
      );
    }
    if (before_id) {
    ...
    const articles = entries.map((entry) => {
      ...
      if (entry.enclosures && entry.enclosures.length > 0) {
        const image = entry.enclosures.find(
          (e) => e.mime_type && e.mime_type.startsWith("image/"),
        );
...
router.post("/batch-read", authenticateToken, async (req, res) => {
  try {
    ...
    await req.miniflux.updateEntriesStatus(
...
router.put("/:id/fetch-content", authenticateToken, async (req, res) => {
  try {
    ...
    if (error.message.includes("404")) {
      console.error(
        "Miniflux returned 404 for fetch-content. Endpoint might not exist or entry ID is wrong.",
      );
      return res.status(404).json({ error: "文章不存在" });
    }
